"""
Sandbox Tool definitions and executor.
Copied from backend/src/services/tools.py with default cwd set to /workspace.
"""
import subprocess
import json
import logging
...
logger = logging.getLogger("Sandbox.Tools")
...
def execute_tool(tool_name: str, tool_input: Dict[str, Any],
                 cwd: str = "/workspace") -> Dict[str, Any]:
    """Execute a tool and return the result."""
    logger.info("[Tools] Executing %s: %s", tool_name, json.dumps(tool_input)[:200])

    try:
        if tool_name == "Bash":
            result = _exec_bash(tool_input, cwd)
        elif tool_name == "Read":
            result = _exec_read(tool_input)
        elif tool_name == "Write":
            result = _exec_write(tool_input)
        elif tool_name == "Edit":
            result = _exec_edit(tool_input)
        elif tool_name == "Glob":
            result = _exec_glob(tool_input, cwd)
        elif tool_name == "Grep":
            result = _exec_grep(tool_input, cwd)
        else:
            result = {"content": f"Unknown tool: {tool_name}", "is_error": True}

        logger.info("[Tools] %s result: is_error=%s, content_len=%d",
                    tool_name, result["is_error"], len(result["content"]))
        if result["is_error"]:
            logger.warning("[Tools] %s error output: %s", tool_name, result["content"][:300])
        return result
    except Exception as e:
        logger.error("[Tools] Error executing %s: %s", tool_name, e, exc_info=True)
...
def _exec_bash(tool_input: Dict, cwd: str) -> Dict[str, Any]:
    ...
    return {"content": output[:MAX_OUTPUT_SIZE], "is_error": result.returncode != 0}
...
def _exec_read(tool_input: Dict) -> Dict[str, Any]:
    ...
    offset = tool_input.get("offset", 1) - 1
